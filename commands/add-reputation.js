...
export const data = new SlashCommandBuilder()
    .setName('add-reputation')
    .setDescription('[АДМИН/ГМ] Добавить репутацию (Rtg) игроку')
    ...
    .addIntegerOption(option =>
        option.setName('amount')
            .setDescription('Количество Rtg (может быть отрицательным)')
            .setRequired(true))
...
export async function execute(interaction) {
    ...
    if (!hasCommandPermission(member, 'add-reputation')) {
    ...
    const msg = await interaction.reply({
        embeds: [createSuccessEmbed('Репутация обновлена',
            `Изменено **${sign}${amount} Rtg** игроку **${name}**.${reasonLine}\n\n` +
