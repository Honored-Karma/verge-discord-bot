...
if (!hasCommandPermission(member, 'set-ap')) {
    ...
    const result = await setAPMultiplier(playerId, multiplier, interaction.user.id, durationSeconds);
    ...
    if (result !== false) {
        ...
        const msg = await interaction.reply({
            embeds: [createSuccessEmbed('Множитель AP установлен', 
                ...
                `${expiresText}\n\n` +
                `**Пример:** Тренировка дает 10 AP → теперь даст **${Math.round(10 * result.multiplier / 100)} AP**`, 'setApMultiplier')],
            fetchReply: true
        });
        autoDeleteMessageShort(msg);
    } else {
