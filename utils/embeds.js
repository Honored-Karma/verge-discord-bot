...
const BANNERS = {
  ...
  cooldown:
    "https://s.iimg.su/s/22/uClf58LxwwC7ZyuLlAWkzTJ3TD4tsoH4fjoKN653.jpg",
  profile:
    "https://s.iimg.su/s/22/ufkWBu0xWZJ35E1dhmIFfmTwEoXZtvP8zOPqXpsI.jpg",
...
function _getActivityBonusPercent(player) {
...
function _getDynamicBonus(player) {
  return _getRankBonusPercent(player.rank) + _getActivityBonusPercent(player);
}
...
export function createModernProfileEmbed(player, user, history = []) {
      ...
      ? history
          .map((entry) => {
            ...
            const timestamp = Math.floor(
              new Date(entry.changed_at).getTime() / 1000,
            );
  ...
  return new EmbedBuilder()
    ...
    .addFields(
      ...
      {
        name: "<:13medal:1494708705759989760>  Ранг",
        value: rankValue,
        inline: true,
      },
      {
        name: "<:AP28112025:1443994380670337245>  AP",
        value: `${player.ap || 0}`,
        inline: true,
      },
      {
        name: "<:SP28112025:1443994403604533268>  SP (всего)",
        value: `${player.total_sp || 0}`,
        inline: true,
      },
      {
        name: "<:Flag_of_South_Korea:1438985777949179935>  KRW",
        value: `${Number(player.krw || 0).toLocaleString("ru-RU")}`,
        inline: true,
      },
      {
        name: "<:2640japanflag:1438980353925714081>  YEN",
        value: `${Number(player.yen || 0).toLocaleString("ru-RU")}`,
        inline: true,
      },
      {
        name: "<:14LightningPurple:1494708639473074277>  Множители",
        value: `AP: **${apMultiplier}%**${apExpireText}\nSP: **${spMultiplier}%**${spExpireText}`,
        inline: true,
      },
...
export function createStylesListEmbed(
  title,
  description,
  page = 1,
  totalPages = 1,
...
export function createProfileMainPage(player, user) {
  ...
  embed.addFields({
    name: "<:13medal:1494708705759989760>  Ранг",
    value: player.rank || "Нет ранга",
    inline: true,
  });
  embed.addFields({
    name: "<:AP28112025:1443994380670337245>  Очки способностей (AP)",
    value: `${player.ap} AP`,
    inline: true,
  });
  ...
  const dynamicBonus = _getDynamicBonus(player);
  const effectiveAPMult = apMult + dynamicBonus;
  const effectiveSPMult = spMult + dynamicBonus;
  let multText = `AP: **${effectiveAPMult}%**`;
  if (apExp > 0) multText += ` (до <t:${apExp}:R>)`;
  multText += ` / SP: **${effectiveSPMult}%**`;
  if (spExp > 0) multText += ` (до <t:${spExp}:R>)`;
  if (dynamicBonus > 0)
    multText += `\n*(база: AP ${apMult}% / SP ${spMult}% + бонус ${dynamicBonus}%)*`;
  embed.addFields({
    name: "<:14LightningPurple:1494708639473074277>  Множители",
    value: multText,
    inline: true,
  ...
  if (player.unlocked_avatar) {
    embed.addFields({
      name: "🌟 Статус",
      value: "Олицетворение достигнуто",
      inline: false,
    });
  }
  ...
  if (player.krw > 0 || player.yen > 0) {
    let currencyText = "";
    if (player.krw > 0)
      currencyText += `<:Flag_of_South_Korea:1438985777949179935>  **${player.krw.toLocaleString("ru-RU")}** KRW\n`;
    if (player.yen > 0)
      currencyText += `<:2640japanflag:1438980353925714081>  **${player.yen.toLocaleString("ru-RU")}** ¥`;
    embed.addFields({
      name: "<:money:1494767683244265652>  Валюта",
      value: currencyText || "Нет валюты",
      inline: false,
    });
  }
...
export function createProfileStylesPage(player, styles, user, page = 0) {
  ...
  embed.setFooter({
    text: `ID: ${player.id} • Страница 3/4 • Стили: ${page + 1}/${totalPages}`,
  });
